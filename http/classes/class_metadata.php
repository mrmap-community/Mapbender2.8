			...
			for($i=0; $i<count($datasetMatrix);$i++){
				...
				foreach ($this->datasetJSON->dataset->srv[$i]->coupledResources->featuretype as $ft) {
					...
					foreach($this->datasetJSON->dataset->srv[$i]->coupledResources->featuretype[$featuretypeCount]->srv->ftype as $ftype) {
						//$e = new mb_exception("classes/class_metadata.php: compare featuretype ids: ".$ftype->id." - ".$ft->id);
						if ($ftype->id !== $ft->id) {
...
/*		#TODO: check if last and first ampersand was set before
		...
		$e = new mb_notice("class_metadata.php: look for pattern: ".$pattern."  in ".$queryString);
 		if (!preg_match($pattern, $queryString, $matches)){
			...
			if (count($matches) == 1) {
				...
				$e = new mb_notice("class_metadata.php: There are parameter ambiguities!");
	...
	private function addToQuery($paramName,$queryString,$string,$queryList) {
		...
		$pattern = ','.$string.',';	
 		if (!preg_match($pattern, $queryListC)){
			...
			//$this->logit("class_metadata.php: queryString_old:".$queryString);
			//$this->logit("class_metadata.php: queryString_new:".$queryStringNew);
			return $queryStringNew;
		} else {
			//$this->logit("class_metadata.php: queryString unchanged:".$queryString);
			//return the old one!
			return $queryString;
		}
	}
