...
//				that.enableShowMetadataLink();
    ...
    this.fillTemplateForm = function (obj) {
        $("select.select-element").bind("change", function () {
			alert(this.val());
		});
        $('fieldset.wmc-template span#wmc_title').text(obj.wmc_title+" ("+obj.wmc_id+")");
        $('fieldset.wmc-template input[name="wmc_id"]').val(obj.wmc_id);
        var targets = obj.elements;
        for(target in targets){
            for(var i = 0; i < targets[target].length; i++){
                var targetObj = targets[target][i];
                for(type in targetObj){
                    for(key in targetObj[type]) {
                        var value = targetObj[type][key];
                        $('fieldset.wmc-template fieldset[data-target="'+target+'"] p[data-type="'+type+'"] input[name="'+key+'"]').val(value);
                    }
                }
            }
        }
...
//			});				
	...
	Mapbender.events.init.register(function () {
		that.valid();
		$("<div></div>").insertAfter($metadataForm).hide().load("../plugins/mb_template_wmc_edit.php");
